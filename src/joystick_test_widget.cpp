...
JoystickTestWidget::JoystickTestWidget(const std::shared_ptr<Joystick>& joystick_)
  : Gtk::Dialog(joystick_->get_name()),
    ...
    button_table((joystick->get_button_count()-1) / 8 + 1, std::min(joystick->get_button_count(), size_t(8))),
    ...
    right_trigger_widget(32, 128, true)
{
  ...
  for (size_t i = 0; i < joystick->get_axis_count(); ++i) {
    ...
    if (i >= axis_table.property_n_rows()) {
  ...
  for (size_t i = 0; i < joystick->get_button_count(); ++i) {
    const int x = i % 8;
    const int y = i / 8;
...
void JoystickTestWidget::axis_move(size_t number, int value) {
...
void JoystickTestWidget::button_move(size_t number, bool value) {
  buttons.at(number)->set_down(value);
}

void JoystickTestWidget::on_calibrate() {
  Main::current()->show_calibration_dialog(joystick);
}

void JoystickTestWidget::on_mapping() {
  Main::current()->show_mapping_dialog(joystick);
}

void JoystickTestWidget::on_response(int) {
  hide();
}

void JoystickTestWidget::on_save_profile() {
  on_save_profile_as("Hello World");
}

void JoystickTestWidget::on_save_profile_as(const std::string& /*name*/) {
...
void JoystickTestWidget::on_delete_profile() {
